...
== Motivation
I want to trace my service calls across service boundries, and server stacks.  To 
do this, I discovered the OpenZipkin project which uses Spring Boot to expose the 
OpenZipkin server. 
...
Follow this http://start.spring.io/starter.zip?dependencies=web,lombok,h2,jpa&type=maven-project&javaVersion=1.8&baseDir=spring-tracing&packageName=mcp.client&name=spring-tracing[preconfigured start.spring.io] link
to download the base project. Find your favorite editor, and start with standing up a REST 
controller. We make this a Spring Boot application that exposes 2 rest endpoints, and utilizes
a `restTemplate` for accessing remote HTTP resources.  In this case, we will expose 1 endpoint
to serve data, and another to act as a first-hop along our service route.
