...
=== Reporting Trace(s) to Zipkin server
...
are found in [grpc-server/pom.xml]. You will want a new project setup with all of the `lognet` and `io.grpc` dependencies and build plugins. 
Once you have this complete, you can use maven to generate sources, and expose the service.
Below is a 'post-generated' listing of our GRPC service. 
...
----
We simply apply the `org.lognet.GRpcService` annotation to mark this bean for service registration.
...
mark the bean as an global interceptor.  This will expose tracing to all GRPC endpoints (visible on the ApplicationContext) in this service.
...
.GrpClientTraceConfiguration.java
[source,java]
----
    @Bean
...
To get zipkin-erver to consume from Kafka, you will need to have a kafka sevice running.
